# https://codechalleng.es/bites/136/
...
def check_bt(donor, recipient):
    ...
    """
    donor = check_input(donor)
    recipient = check_input(recipient)
    if -1 in _particular_antigen_comp(donor, recipient):
        return False
    else:
        return True
...
def check_input(input):
    if type(input) is str:
        if input in blood_type_text.keys():
            return blood_type_text[input].value
        else:
            raise ValueError
        ...
        if -1 < input < 8:
            ...
            raise ValueError
    elif isinstance(input, Bloodtype):
        return input.value
    else:
        raise TypeError
