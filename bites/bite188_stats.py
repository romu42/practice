# https://codechalleng.es/bites/188/
...
def get_all_line_counts(data: str = STATS) -> list:
    ...
    with open(data) as file:
        bob = file.read()
    for line in bob.split('\n'):
        input_list.append(line.lstrip().split(' ')[0])
    return list(map(int, filter(None, input_list)))
...
if __name__ == '__main__':
    get_all_line_counts(STATS)
    counts = list(get_all_line_counts(STATS))
    print(len(counts))
    print(create_stats_report(counts))